...
function makeWord(lettersObject) {
  ...
  const maxSize = Math.max(...Object.values(lettersObject).flat()) + 1;

  if (maxSize <= 0 || maxSize > Number.MAX_SAFE_INTEGER) {
    throw new RangeError('Invalid array length');
  }

  const arr = new Array(maxSize).fill('');
...
class Selector {
  ...
  static combine(selector1, combinator, selector2) {
    const combinedSelector = new Selector();
  ...
  checkOrder(weight) {
    ...
    if (weight <= this.lastWeight && [1, 2, 6].includes(weight)) {
      throw new Error(this.errorOneTimeUse);
    }
    this.lastWeight = weight;
  }

  element(value) {
    this.checkOrder(this.weights.element);
    this.selector += `${value}`;
    return this;
  }

  id(value) {
    this.checkOrder(this.weights.id);
    this.selector += `#${value}`;
    return this;
  }

  class(value) {
    this.checkOrder(this.weights.class);
    this.selector += `.${value}`;
    return this;
  }

  attr(value) {
    this.checkOrder(this.weights.attr);
    this.selector += `[${value}]`;
    return this;
  }

  pseudoClass(value) {
    this.checkOrder(this.weights.pseudoClass);
    this.selector += `:${value}`;
    return this;
  }

  pseudoElement(value) {
    this.checkOrder(this.weights.pseudoElement);
    this.selector += `::${value}`;
    return this;
  }
